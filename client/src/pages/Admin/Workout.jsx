...
import { HiPencilSquare, HiTrash, HiMiniPlusCircle } from "react-icons/hi2";
  ...
  const handleConfirmModelConfirm = async () => {
    ...
    try {
      const response = await axios.delete(
        `${appConstants.SERVER_URL}/api/workouts/${selectedRecord}`,
        {
          headers: {
    ...
    <div className="">
      <div className="px-4 space-y-4">
        <h1 className="mb-8 text-4xl tracking-tight font-extrabold text-gray-800 dark:text-white flex justify-between">
          <span>Workouts</span>
          <Link
            to={`/admin/dashboard?tab=workoutForm`}
                  ...
                  <tr key={index}>
                    ...
                    <td>
                      <div className="flex space-x-2">
                        <Link
                          to={`/admin/dashboard?tab=workoutForm&workoutId=${record._id}`}
                        >
